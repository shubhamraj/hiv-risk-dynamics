...
public class EpisodicModel implements BaseModelInterface {
	...
	private EnumMap<RiskState, ArrayList<EpisodicRiskAgent>> riskStates;
	...
	public void setup() {
		...
		riskStates = new EnumMap<RiskState, ArrayList<EpisodicRiskAgent>>(RiskState.class);
		
		countTot = 0; countHigh = 0; countLow = 0; countHL = 0; inlow=0; inhi=0; inlh=0; ;
		
		initializeProbabilities();

		for (RiskState riskState : RiskState.values()) {
	...
	private void mixing() {
		int numHighRisk = riskStates.get(RiskState.High).size();
		int numLowRisk = riskStates.get(RiskState.Low).size();
		...
		for (EpisodicRiskAgent individual : riskStates.get(RiskState.High)) {
		...
		for(EpisodicRiskAgent individual : riskStates.get(RiskState.Low)) {
		...
		for (EpisodicRiskAgent low1 : lowRisks) {
			if (Uniform.staticNextDouble() <= (p2)) {
				low2 = (EpisodicRiskAgent) riskStates.get(RiskState.High).get(Uniform.staticNextIntFromTo(0, numHighRisk-1));
			}
			else {
				do {
			...
			countLow++;
			if (sexualTransmission(low1, low2, MixingSite.Common) && currentTick >= 29990) {
				inlow++;
			}
			countTot++;
		}				
		...
		if (hTol.size() > 1) {
			try {
				for (EpisodicRiskAgent ind1 : hTol) {
					...
					countHL++;
					if (sexualTransmission(ind1, ind2, MixingSite.Common) && currentTick >= 29990) {		
						inlh++;
					};
					countTot++;
				}							
		...
		for (EpisodicRiskAgent ind1 : hToh) {
			do {
				ind2 = (EpisodicRiskAgent) riskStates.get(RiskState.High).get(Uniform.staticNextIntFromTo(0, numHighRisk-1));
				//ind2 = (Individual) hToh.get(Uniform.staticNextIntFromTo(0, hToh.size()-1));
			} while (ind1.equals(ind2));
			countHigh++;
			if(sexualTransmission(ind1, ind2, MixingSite.HighRisk) && currentTick >= 29990) {
				inhi++;
			}
			countTot++;
		}
	...
	private boolean sexualTransmission(EpisodicRiskAgent agent1, EpisodicRiskAgent agent2, MixingSite mixingSite) {
	...
	void infect(EpisodicRiskAgent infector, EpisodicRiskAgent susceptible, MixingSite mixingSite) {
		double prob = 0;
	...
	private void riskTransitions() {
		...
		for (EpisodicRiskAgent individual : individuals) {
			...
			if (curState.equals(RiskState.High)) {				
				if (rand <= probHighToLow) {
					newState = RiskState.Low;
				}
			}
			else if (curState.equals(RiskState.Low)) {
				if (rand <= probLowToHigh) {
					newState = RiskState.High;
				}
			}
			else {
	...
	public void createIndividual() {
		...
		RiskState riskState = RiskState.Low;
	...
	private void ud() {
		...
		for (EpisodicRiskAgent individual : individuals) {			
			if (individual.isInfected()) {
				hiv++;
				if (individual.getRiskState().equals(RiskState.High)) {
					highHIV++;
				}
				else {
					lowHIV++;
		...
		double popHigh = riskStates.get(RiskState.High).size();
		double popLow = riskStates.get(RiskState.Low).size();
